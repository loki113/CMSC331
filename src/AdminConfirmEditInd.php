  ...
  <body>
    <div id="login">
      <div id="form">
		...
		// retrieves information about advisor based on name
            $sql = "SELECT `id` FROM `Proj2Advisors` WHERE `FirstName` = '$row[1]' AND `LastName` = '$row[2]'";
            $rs = $COMMON->executeQuery($sql, "Advising Appointments");
            $rod = mysql_fetch_row($rs);
            $adv = $rod[0];

		// retrieves information about student based on student ID
            if($row[4]){
		...
		// displays information about appointment
	...
	<div class="bottom">
		<?php
		// displays whether or not the student has been informed of the appointment being cancelled
		if($row[4]){
